...
common_chat_msg common_chat_peg_parse(const common_peg_arena &          src_parser,
    ...
    if (result.fail()) {
        ...
        // Story S15: return a content-only fallback msg instead of throwing.
        // The autoparser-generated PEG grammar for the model's chat template
        // can fail to match degenerate model output (long-ctx drift, greedy
        // decoding, low-bit quantization). Previously this threw an uncaught
        // std::runtime_error that propagated to libc++abi::terminate -> SIGABRT,
        // aborting llama-cli -cnv mid-stream. Returning the raw text as plain
        // assistant content degrades gracefully: generation stays bounded by
        // n_predict, antiprompt/EOS detection runs as usual, and the caller
        // still sees the full generated text. Mirrors the partial-results
        // graceful path above. The underlying degeneration is a model/template
        // quality issue, not a parser issue — but the parser must not abort.
        common_chat_msg fallback;
        fallback.role    = "assistant";
        fallback.content = effective_input;
        return fallback;
    }
