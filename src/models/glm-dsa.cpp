...
void llama_model_glm_dsa::load_arch_hparams(llama_model_loader & ml) {
    ...
    // (n_ff_exp and n_expert_shared already loaded in the MoE block above —
    // REMEDIATION_PLAN P3 #6: removed the duplicate MLA-block loads here.)
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self_attention
        {
            ...
            if (is_full_indexer_layer(il)) {
                ...
                // Guard against silent token loss: ggml_view_4d divides ne[2] by
                // n_stream (integer division). If n_tokens is not a multiple of
                // n_stream the trailing tokens are silently truncated. Batch-1
                // decode (n_stream==1) is unaffected. See REMEDIATION_PLAN P2.
